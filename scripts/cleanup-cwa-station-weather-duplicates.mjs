#!/usr/bin/env node
/**
 * scripts/cleanup-cwa-station-weather-duplicates.mjs
 *
 * One-time cleanup for cwa_station_weather: the UNIQUE KEY includes obs_time,
 * so every sync cycle that sees a genuinely new obs_time INSERTs a new row
 * instead of overwriting the old one. Confirmed on production 2026-09-21:
 * 1,856,771 rows across 876 stations (some stations at 3,456 rows each,
 * accumulated since 2026-07-29), and the nearest-station query's haversine
 * ORDER BY can't use any index against a table this size (EXPLAIN showed
 * type=ALL + Using filesort, ~812k rows scanned) — this is what took down
 * every DB-backed API route on 2026-09-21 (see
 * docs/specs/cwa-station-weather-duplicate-row-cleanup.md).
 *
 * This script keeps only the newest obs_time row per (dataset_id, station_id)
 * and deletes the rest, in small batches to avoid holding a long lock on a
 * live table that a sync cron is also writing to. It backs up the full table
 * before deleting anything.
 *
 * Usage:
 *   node scripts/cleanup-cwa-station-weather-duplicates.mjs                # dry-run (default)
 *   node scripts/cleanup-cwa-station-weather-duplicates.mjs --execute      # actually delete
 *   node scripts/cleanup-cwa-station-weather-duplicates.mjs --execute --batch-size=2000
 *   node scripts/cleanup-cwa-station-weather-duplicates.mjs --execute --no-backup
 *   node scripts/cleanup-cwa-station-weather-duplicates.mjs --remote       # force SSH path
 *
...
const TABLE = "cwa_station_weather";
...
async function cleanup(conn, { dryRun, batchSize, backup }) {
  ...
  await conn.query(`
    ...
    SELECT dataset_id, station_id, MAX(obs_time) AS max_obs_time
    FROM ${TABLE}
    GROUP BY dataset_id, station_id
  `);
...
async function main() {
  console.log("==================================================================");
  console.log("  cwa_station_weather 重複列清理工具");
  console.log("==================================================================");
  console.log(`模式: ${DRY_RUN ? "🔍 [DRY-RUN] 僅預覽，不刪除任何資料" : "⚡ [EXECUTE] 正式刪除重複列"}`);
  console.log(`批次大小: ${BATCH_SIZE}`);
  if (!DRY_RUN) console.log(`備份: ${NO_BACKUP ? "❌ 已停用 (--no-backup)" : "✅ 刪除前會先建立完整備份表"}`);
  ...
  try {
    const res = await cleanup(conn, { dryRun: DRY_RUN, batchSize: BATCH_SIZE, backup: !NO_BACKUP });
    await conn.end();
    report(res);
  } catch (err) {
...
function report(res) {
  ...
  if (res.backupTable) {
    ...
    console.log(`⚠️ 未建立備份表 (--no-backup)`);
  }
  console.log(`\n🎉 清理完成！`);
}

...
export async function runEarthquakeSync(): Promise<EarthquakeSyncResult[]> {
  ...
  for (const source of SOURCES) {
    try {
      ...
      results.push({ sourceKey: source.key, fetched: events.length, inserted, matched, error: null });
    } catch (error) {
      results.push({
        ...
        inserted: 0,
        matched: 0,
        error: error instanceof Error ? error.message : "Unknown earthquake sync error",
      });
    }
  }

...
// ---------------------------------------------------------------------------
// Several hospitals in one article (issue #87)
// ---------------------------------------------------------------------------

/**
 * The whole tier-1 decision for a piece of copy, minus the database.
 *
 * Each matching alias is resolved to a row named exactly its searchName, which
 * is what the real lookup returns: #84 made every searchName an exact-name
 * `clinic` row, and scripts/verify-hospital-search-names.mjs keeps it that way
 * against production. Returns the winning name, or null when tier 1 declines.
 */
const tier1 = (text) =>
  selectUniqueInstitution(
    [
      ...new Set(
        COMMON_HOSPITAL_PATTERNS.filter(({ regex }) => regex.test(text)).map(
...
test("no hospital named: declines, and the waterfall falls through as before", () => {
  assert.equal(selectUniqueInstitution([]), null);
  assert.equal(tier1("行政院會今日通過長照三點零計畫，預算增列兩百億元"), null);
});

test("one hospital named: unchanged", () => {
  assert.equal(tier1("臺中榮民總醫院發表最新研究"), "臺中榮民總醫院");
  assert.equal(
    tier1("衛福部桃園醫院院內感染調查結果出爐"),
    "衛生福利部桃園醫院",
  );
});

test("same institution reached twice is not two institutions", () => {
  // /news/779356, /news/720110: 「淡水馬偕紀念醫院」 fires /淡水馬偕/ AND the
  // bare /馬偕醫院|馬偕紀念醫院/. One hospital, two regexes — the specific
  // branch must win by subsumption, NOT be discarded as ambiguous. Three of
  // the ten multi-match articles in the live sample are this shape, so a naive
  // "several matches → decline" rule would throw away a correct unique answer
  // 30% of the time.
  assert.equal(
    tier1("淡水馬偕紀念醫院急診室今日啟用新空間"),
    "台灣基督長老教會馬偕醫療財團法人淡水馬偕紀念醫院",
  );
  // /news/627919
  assert.equal(
  ...
  // Same shape one family over: 「三軍總醫院北投分院」 fires the 北投 entry and
  // the bare /三軍總醫院|三總/ one. The registry names interleave 分院 rather
  // than appending it, which is why plain substring containment cannot see it.
  assert.equal(
    tier1("三軍總醫院北投分院精神科門診"),
    "三軍總醫院北投分院附設民眾診療服務處",
  );
  // And a bare mention still resolves to the flagship on its own.
  assert.equal(
    tier1("馬偕紀念醫院公布數據"),
    "台灣基督長老教會馬偕醫療財團法人馬偕紀念醫院",
  );
...
test("two genuinely different hospitals: declines", () => {
  // /news/874073
  assert.equal(tier1("臺中榮民總醫院與林口長庚紀念醫院共同發表"), null);
  // /news/877122
  assert.equal(tier1("臺中榮總、部立桃園醫院同步啟動"), null);
  // /news/727150
  assert.equal(tier1("臺中榮民總醫院與三軍總醫院參與試驗"), null);
});

test("three genuinely different hospitals: declines", () => {
  // /news/879546
  assert.equal(
    tier1("臺中榮民總醫院、高雄長庚、衛福部桃園醫院聯手投入聯邦學習"),
    null,
  );
});

test("two branches of one family: declines, no parent row to collapse to", () => {
  // /news/855243. #84 measured that the facilities table holds no exact-name
  // row for 佛教慈濟醫療財團法人 — the family exists only as branches — so
  // 「大林慈濟 + 斗六慈濟」 has nowhere to be redirected to and declines like
  // any other multi-institution article.
  assert.equal(tier1("大林慈濟醫院與斗六慈濟醫院聯合說明"), null);
  assert.equal(tier1("林口長庚與高雄長庚同步導入"), null);
  assert.equal(tier1("台北榮總與高雄榮總"), null);
});

test("subsumption never fires between different institutions", () => {
  // The relation is a subsequence test, which is looser than substring; this
  // pins down that its looseness stays inside a family. Across the 53 alias
  // entries it holds for exactly seven pairs — the four 三軍總醫院 branches and
  // the three city-prefixed 馬偕 ones, each against its own bare family entry.
  const names = [
    ...new Set(COMMON_HOSPITAL_PATTERNS.map(({ searchName }) => searchName)),
  ];
  const subsumed = [];
  for (const a of names) {
    for (const b of names) {
      if (a === b) continue;
      if (selectUniqueInstitution([{ name: a }, { name: b }])) {
        subsumed.push(`${a} / ${b}`);
      }
    }
  }
  // Each of the 7 pairs shows up twice, once per ordering.
  assert.equal(subsumed.length, 14, subsumed.join("\n"));
  for (const pair of subsumed) {
    assert.ok(
      /三軍總醫院/.test(pair) || /馬偕/.test(pair),
      `unexpected subsumption: ${pair}`,
    );
  }
});

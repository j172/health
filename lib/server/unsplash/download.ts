...
import {
  ...
  type UnsplashImage,
} from "@/lib/server/unsplash/client";
...
const triggerUnsplashDownload = async (
  ...
  try {
    ...
    console.warn(
      `[unsplash] Failed to ping download_location (non-fatal): ${error instanceof Error ? error.message : String(error)}`,
    );
  }
};

export const downloadUnsplashImage = async (
  image: UnsplashImage,
): Promise<DownloadedUnsplashImage> => {
  ...
  if (!extension) {
    throw new Error(
      `Unsplash image has unsupported content type: ${mime || "unknown"}.`,
    );
  }

...
test("the nine productive site: feeds are kept — the gate handles their noise", () => {
  const codes = new Set(RSS_FEEDS.map((feed) => feed.code));
  for (const kept of [
    "ubrand_udn",
    "esg_gvm",
...
test("ntuh_news and ntuh_ifc_news are removed from RSS_FEEDS and SOURCE_CATEGORIES", async () => {
  const codes = new Set(RSS_FEEDS.map((feed) => feed.code));
  assert.equal(codes.has("ntuh_news"), false, "ntuh_news must be removed");
  assert.equal(codes.has("ntuh_ifc_news"), false, "ntuh_ifc_news must be removed");

  const { SOURCE_CATEGORIES } = await import("../news/sourceCategories.ts");
  const govCat = SOURCE_CATEGORIES.find((c) => c.key === "gov");
  assert.ok(govCat, "gov category must exist");
  assert.equal(
    govCat.sources.some((s) => s.sourceName === "ntuh" || s.sourceName === "ntuh_ifc"),
    false,
    "ntuh and ntuh_ifc should not be present in gov category",
  );
});

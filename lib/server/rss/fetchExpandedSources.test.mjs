...
test("Phase 3: parseMoeFamilyEduHtml parses official government listings", () => {
  const moeHtml = `
...
test("sfaa is registered in RSS_FEEDS with its full-text-yielding RSS URL (issue #355 migration), not the HTML list-page scrape", () => {
  const sfaa = RSS_FEEDS.find((f) => f.code === "sfaa_news");
  assert.ok(sfaa, "RSS_FEEDS must contain sfaa_news");
  assert.equal(sfaa?.sourceName, "sfaa");
  assert.equal(sfaa?.url, "https://www.sfaa.gov.tw/SFAA/RSS.aspx?type=1");

  const freeway = RSS_FEEDS.find((f) => f.code === "freeway_news");
  assert.ok(freeway, "RSS_FEEDS must contain freeway_news (migrated from the retired gov-opendata pipeline)");
  assert.equal(freeway?.sourceName, "freeway");
  assert.equal(freeway?.url, "https://www.freeway.gov.tw/rss.aspx");
});

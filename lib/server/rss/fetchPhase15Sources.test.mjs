...
test("mababy is removed as an active source", () => {
  const mediaCategory = SOURCE_CATEGORIES.find((c) => c.key === "media");
  assert.ok(mediaCategory, "media category must exist");
  assert.equal(
    mediaCategory.sources.some((source) => source.sourceName === "mababy"),
    false,
    "mababy should not be present in the media category",
  );
});

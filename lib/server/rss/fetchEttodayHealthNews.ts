...
const parseRelativeTaipeiTime = (text: string, now: Date): Date | null => {
  ...
  if (hourMatch) return new Date(now.getTime() - Number(hourMatch[1]) * 60 * 60 * 1000);

  const minuteMatch = trimmed.match(/^(\d+)\s*分鐘前$/);
  if (minuteMatch) return new Date(now.getTime() - Number(minuteMatch[1]) * 60 * 1000);

  const dayMatch = trimmed.match(/^(\d+)\s*天前$/);
  if (dayMatch) return new Date(now.getTime() - Number(dayMatch[1]) * 24 * 60 * 60 * 1000);
...
export const fetchEttodayHealthNews = async (): Promise<EttodayHealthFetchResult> => {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Unknown ETtoday health fetch error";
    return {
      ok: false,
      httpStatus,
      itemCount: 0,

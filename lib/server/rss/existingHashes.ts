...
export const itemKey = (item: Pick<NormalizedRssItem, "sourceName" | "feedCode" | "externalId">): string =>
...
export const getExistingPayloadHashes = async (items: NormalizedRssItem[]): Promise<Map<string, string>> => {
  if (items.length === 0) return new Map();

  return withConnection(async (conn) => {
    const result = new Map<string, string>();
    const bySource = new Map<string, { sourceName: string; feedCode: string; externalIds: string[] }>();

    for (const item of items) {
      const groupKey = `${item.sourceName}::${item.feedCode}`;
      const group = bySource.get(groupKey) ?? { sourceName: item.sourceName, feedCode: item.feedCode, externalIds: [] };
    ...
    for (const { sourceName, feedCode, externalIds } of bySource.values()) {
      ...
      for (const row of rows) {
        result.set(`${sourceName}::${feedCode}::${row.external_id}`, row.payload_hash);
      }
    }

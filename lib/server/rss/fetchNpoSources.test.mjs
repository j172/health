...
const { parseYmdToUtc, fetchApatwNews, fetchSeinsightsNews, fetchMygopenNews, fetchKumaNews } = await import("./fetchNpoSources.ts");

test("NPO: fetchApatwNews, fetchSeinsightsNews, fetchMygopenNews, and fetchKumaNews are exported and are functions", () => {
  assert.equal(typeof fetchApatwNews, "function");
  assert.equal(typeof fetchSeinsightsNews, "function");
  assert.equal(typeof fetchMygopenNews, "function");
  assert.equal(typeof fetchKumaNews, "function");
});
...
test("NPO: SOURCE_CATEGORIES contains 'npo' (公益社福) category with all required sources including apatw, seinsights, and mygopen", () => {
...
test("NPO: fetchKumaNews successfully fetches and parses Kuma Academy news articles", async () => {
  const res = await fetchKumaNews();
  assert.ok(res, "fetchKumaNews should return a result object");
  assert.equal(res.ok, true, "fetchKumaNews should succeed");
  assert.ok(res.itemCount > 0, "should have fetched articles");
  assert.equal(res.items[0].sourceName, "kuma");
  assert.equal(res.items[0].feedCode, "kuma_news");
  assert.equal(res.items[0].feedName, "黑熊學院");
  assert.ok(res.items[0].title.length > 0);
  assert.ok(res.items[0].canonicalUrl.startsWith("https://kuma-academy.org/article/"));
  assert.ok(res.items[0].publishedAtUtc instanceof Date);
});

...
const { compareByStrokeOrder } = await import("./strokeOrder.ts");
const { facilitySearchConfigs } = await import("../../../app/tools/facilityConfigs.ts");

// issue #256: reclassified from 7 groups to 9 — TOOL_GROUP_META in catalog.ts
// is the single source of truth Nav/Footer/`/tools` all derive from now.
const TOOL_GROUPS = [
  "calculator",
  "care-facility",
  "registry",
...
test("all 9 tool groups in SiteNav/SiteFooter are sorted by the stroke-order rule", () => {
  // spec: navbar-footer-reclassification-and-merges §0.2 — Latin-named items
  // form one alphabetical block ahead of every Chinese label, then Chinese
  // items sort by first-character stroke count ascending. `toolsInGroup`
  // applies `compareByStrokeOrder` internally; this test just confirms the
  // order it returns actually satisfies that rule (not the old plain
  // zh-Hant collation the 7-group layout used).
  for (const group of TOOL_GROUPS) {
    const tools = toolsInGroup(group);
    assert.ok(tools.length > 0, `Group "${group}" has no tools`);

    const titles = tools.map((t) => t.title);
    const sortedTitles = [...titles].sort(compareByStrokeOrder);

    assert.deepEqual(
      titles,
      sortedTitles,
      `Tool group "${group}" is not sorted by the stroke-order rule`
    );
  }
});
...
test("overview links and brand social URLs are well-formed", () => {
  const overviewHrefs = ["/", "/news", "/privacy", "/llm-info"];
...
test("civic partner links in SiteFooter are valid HTTPS URLs with target=_blank", () => {
  const footerContent = readFileSync(path.join(ROOT_DIR, "components", "News", "SiteFooter.tsx"), "utf-8");
  const expectedUrls = [
    "https://g0v.tw/intl/zh-TW/",
    "https://kuma-academy.org/",
    "https://cw.yueyuknows.com/",
    "https://metawilo.com/",
    "https://council2026.taiwangogo.tw/",
  ];
  for (const url of expectedUrls) {
    assert.ok(footerContent.includes(url), `SiteFooter.tsx must contain civic partner URL: ${url}`);
  }
  assert.ok(footerContent.includes("footer.civicPartners"), "SiteFooter must contain footer.civicPartners i18n key");
});

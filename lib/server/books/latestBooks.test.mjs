...
const { parseBooksComTwHtml, parseEsliteHtml, parseTaazeRssXml } = await import("./scraper.ts");
...
test("Phase 4: BOOKS_CATEGORIES contains all 4 博客來, 27 誠品, and 10 TAAZE categories (41 total)", () => {
  assert.equal(BOOKS_CATEGORIES.length, 41, "Total categories must be exactly 41");

  const taazeCats = getCategoriesByPlatform("taaze");
  assert.equal(taazeCats.length, 10, "Must have exactly 10 TAAZE categories");
  const taazeIds = taazeCats.map((c) => c.id);
  assert.ok(taazeIds.includes("taaze_5_16"), "Must include 注目新書－醫學保健");
  assert.ok(taazeIds.includes("taaze_6_16"), "Must include 編輯推薦－醫學保健");
...
test("Phase 4: parseTaazeRssXml correctly parses TAAZE RSS item XML", () => {
  const sampleXml = `<?xml version="1.0" encoding="utf-8"?>
<rss version="2.0"><channel><item>
<title>父母老了，我也老了：悉心看顧、適度喘息，關懷爸媽的全方位照護指南</title>
<link>http://www.taaze.tw/sing.html?pid=11100907396</link>
<description>&lt;![CDATA[&lt;table&gt;&lt;tr&gt;&lt;td&gt;&lt;a href="http://www.taaze.tw/sing.html?pid=11100907396"&gt;&lt;img border="0" src="http://media.taaze.tw/showProdImage.html?sc=11100907396" /&gt;&lt;/a&gt;&lt;/td&gt;&lt;td width="5"&gt;&lt;/td&gt;&lt;td valign="top"&gt;父母老了，我也老了&lt;br /&gt;出版日期&amp;nbsp;:&amp;nbsp;&amp;nbsp;2020-05-16&lt;br /&gt;定價&amp;nbsp;:&amp;nbsp;&amp;nbsp;380&amp;nbsp;元,&amp;nbsp;&amp;nbsp;&amp;nbsp;&amp;nbsp;&lt;font color="color:#472a2b;"&gt;特價&amp;nbsp;:&amp;nbsp;&amp;nbsp;300&amp;nbsp;元&lt;/font&gt;&lt;br /&gt;&lt;/td&gt;&lt;/tr&gt;&lt;/table&gt;]]&gt;</description>
<pubDate>Mon, 18 May 2020 01:17:54 +0800</pubDate>
</item></channel></rss>`;

  const cat = getCategoryById("taaze_5_16");
  assert.ok(cat);
  const items = parseTaazeRssXml(sampleXml, cat);
  assert.equal(items.length, 1);
  assert.equal(items[0].platform, "taaze");
  assert.equal(items[0].title, "父母老了，我也老了：悉心看顧、適度喘息，關懷爸媽的全方位照護指南");
  assert.equal(items[0].publishDate, "2020-05-16");
  assert.equal(items[0].listPrice, 380);
  assert.equal(items[0].salePrice, 300);
  assert.equal(items[0].discount, "79折");
  assert.ok(items[0].productUrl.startsWith("https://"));
  assert.ok(items[0].coverUrl.startsWith("https://"));
});

test("Phase 4 / issue #256: latest-books is registered in TOOL_CATALOG under 'culture-tourism' group", () => {
  const entry = getToolCatalogEntry("latest-books");
  assert.ok(entry, "latest-books must exist in TOOL_CATALOG");
  assert.equal(entry.group, "culture-tourism", "latest-books must belong to culture-tourism (文化藝術與觀光)");
  assert.equal(entry.title, "書籍推薦：博客來、誠品與TAAZE讀冊選書");

...
export default function ClinicsContent() {
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({ type: "clinic" });
    if (keyword) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="mb-2 text-3xl font-bold text-neutral-800 md:text-4xl">🏥 醫療院所查詢</h1>
        <p className="text-neutral-600">查詢全民健保特約醫療院所。資料來源：衛福部中央健康保險署（目前收錄醫學中心、區域醫院、地區醫院）。</p>
      </div>
      ...
      {error && <div className="rounded-none bg-red-50 p-4 text-sm text-red-700">查詢院所資料失敗，請稍後再試。</div>}
      ...
      {!loading && !location.loading && !error && facilities && (
        <>
          ...
          {facilities.length === 0 ? (
            <p className="py-8 text-center text-neutral-500">{keyword ? "查無符合的院所。" : "附近查無已定位的院所，可改用關鍵字搜尋。"}</p>
          ) : (

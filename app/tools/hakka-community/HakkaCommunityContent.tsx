...
export default function HakkaCommunityContent() {
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({ type: "hakka_community" });
    if (keyword) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="mb-2 text-3xl font-bold text-neutral-800 md:text-4xl">🏘️ 客庄社區發展協會查詢</h1>
        <p className="text-neutral-600">查詢客家委員會客庄社區發展協會名冊。資料來源：客家委員會開放資料。</p>
      </div>
      ...
      {error && <div className="rounded-none bg-red-50 p-4 text-sm text-red-700">查詢協會資料失敗，請稍後再試。</div>}
      ...
      {!loading && !location.loading && !error && facilities && (
        <>
          ...
          {facilities.length === 0 ? (
            <p className="py-8 text-center text-neutral-500">{keyword ? "查無符合的協會。" : "附近查無收錄的客庄社區發展協會，可改用關鍵字搜尋。"}</p>
          ) : (

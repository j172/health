...
export default function LongTermCareContent() {
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({ type: "long_term_care" });
    if (keyword) {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-neutral-600">查詢衛福部許可全台長期照顧服務機構。資料來源：衛福部長照服務地圖。</p>
      </div>
      ...
      {!loading && !location.loading && !error && facilities && (
        <>
          ...
          {facilities.length === 0 ? (
            <p className="py-8 text-center text-neutral-500">{keyword ? "查無符合的機構。" : "附近查無收錄的長照機構，可改用關鍵字搜尋。"}</p>
          ) : (

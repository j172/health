...
export default function SleepAssessment() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="mb-2 text-3xl font-bold text-neutral-800 dark:text-slate-100 md:text-4xl">😴 睡眠品質評估</h1>
        <p className="text-neutral-600 dark:text-slate-300">基於 PSQI 量表 7 個面向，評估您的睡眠狀況並提供科學化睡眠衛生改善建議。</p>
      </div>
      ...
      {!submitted &&
        questions.map((q, idx) => (
          <div key={q.id} className="space-y-2">
            <p className="text-sm font-medium text-neutral-800 dark:text-slate-100">
              <span className="text-neutral-500 dark:text-slate-400">{idx + 1}. </span>
              {q.text}
            </p>
                ...
                <button
                  ...
                  className={`rounded-lg border px-3 py-2 text-left text-sm transition-all ${
                    answers[q.id] === opt.value
                      ? "border-primary bg-zumthor font-medium text-primary dark:bg-primary/20 dark:border-primary"
                      : "border-neutral-300 hover:border-primary/50 dark:border-slate-700 dark:text-slate-200 dark:hover:border-primary/50"
                  }`}
                >
                  {opt.label}
                </button>
        ...
        <div className="space-y-4">
          <div className={`${result.bgColor} space-y-3 rounded-xl border border-neutral-200 p-5 dark:border-slate-800`}>
            <div className="space-y-1 text-center">
              <p className="text-xs text-neutral-500 dark:text-slate-400">PSQI 參考分數（0–21）</p>
              <p className="text-5xl font-bold tabular-nums text-neutral-800 dark:text-slate-100">{result.score}</p>
              <p className={`text-lg font-semibold ${result.levelColor}`}>{result.level}</p>
            </div>

            <div className="space-y-1.5 pt-2">
              <p className="text-sm font-medium text-neutral-800 dark:text-slate-100">改善建議：</p>
          ...
          <div className="space-y-1 border-t border-neutral-200 pt-3 text-xs text-neutral-500 dark:border-slate-800 dark:text-slate-400">

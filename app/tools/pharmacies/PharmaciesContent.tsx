...
export default function PharmaciesContent() {
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({ type: "pharmacy" });
    if (keyword) {
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSearch} className="flex gap-2">
        <input
          ...
          placeholder="輸入藥局名稱或地址關鍵字（縣市、鄉鎮）"
      ...
      {!keyword && location.isDefault && !location.loading && <p className="text-xs text-neutral-500">⚠️ 無法取得您的定位，附近搜尋改用預設位置範圍；您也可以直接用上方關鍵字搜尋全台藥局。</p>}
      ...
      {error && <div className="rounded-none bg-red-50 p-4 text-sm text-red-700">查詢藥局資料失敗，請稍後再試。</div>}

      {!loading && !location.loading && !error && facilities && (
        <>
          {markers.length > 0 && (
            <div className="h-[400px] overflow-hidden rounded-none border border-neutral-200">
              <FacilityMap userLocation={location} markers={markers} radiusMeters={5000} showRadius={!keyword} />
            </div>
          )}

          {facilities.length === 0 ? (
            <p className="py-8 text-center text-neutral-500">{keyword ? "查無符合的藥局。" : "附近 5 公里內查無已定位的藥局，可改用關鍵字搜尋。"}</p>
          ) : (

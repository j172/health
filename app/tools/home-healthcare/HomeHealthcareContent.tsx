...
export default function HomeHealthcareContent() {
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({ type: "home_healthcare" });
    if (keyword) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="mb-2 text-3xl font-bold text-neutral-800 md:text-4xl">🏠 居家醫療查詢</h1>
        <p className="text-neutral-600">查詢提供居家醫療照護服務的全民健保特約機構。資料來源：衛福部中央健康保險署。</p>
      </div>
      ...
      {!loading && !location.loading && !error && facilities && (
        <>
          ...
          {facilities.length === 0 ? (
            <p className="py-8 text-center text-neutral-500">{keyword ? "查無符合的機構。" : "附近查無收錄的機構，可改用關鍵字搜尋。"}</p>
          ) : (

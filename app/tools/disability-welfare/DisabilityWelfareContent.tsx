...
export default function DisabilityWelfareContent() {
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({ type: "disability_welfare" });
    if (keyword) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="mb-2 text-3xl font-bold text-neutral-800 md:text-4xl">🧑‍🦽 身心障礙福利機構查詢</h1>
        <p className="text-neutral-600">查詢衛福部全國身心障礙福利機構名冊。資料來源：衛福部社會及家庭署開放資料。</p>
      </div>
      ...
      {!loading && !location.loading && !error && facilities && (
        <>
          ...
          {facilities.length === 0 ? (
            <p className="py-8 text-center text-neutral-500">{keyword ? "查無符合的機構。" : "附近查無收錄的身心障礙福利機構，可改用關鍵字搜尋。"}</p>
          ) : (
            <div className="space-y-3">
              ...
              {facilities.map((f) => (
                <div key={f.id} className="rounded-none border border-neutral-200 p-4">
                  ...
                  {f.service_item && <p className="mt-1 text-xs text-neutral-500">機構類型：{f.service_item}</p>}
                </div>
              ))}
            </div>
          )}

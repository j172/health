...
export default function LtcContractedContent() {
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({ type: "ltc_contracted" });
    if (keyword) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="mb-2 text-3xl font-bold text-neutral-800 md:text-4xl">🤝 長照特約服務機構查詢</h1>
        <p className="text-neutral-600">查詢衛福部長照2.0特約服務機構，涵蓋居家服務、日間照顧、喘息服務等契約服務項目。資料來源：衛福部長照服務資訊網。</p>
      </div>
      ...
      {!loading && !location.loading && !error && facilities && (
        <>
          ...
          {facilities.length === 0 ? (
            <p className="py-8 text-center text-neutral-500">{keyword ? "查無符合的機構。" : "附近查無收錄的長照特約服務機構，可改用關鍵字搜尋。"}</p>
          ) : (
            <div className="space-y-3">
              ...
              {facilities.map((f) => (
                <div key={f.id} className="rounded-none border border-neutral-200 p-4">
                  ...
                  {f.service_item && <p className="mt-1 text-xs text-neutral-500">特約服務項目：{f.service_item}</p>}
                </div>
              ))}
            </div>
          )}

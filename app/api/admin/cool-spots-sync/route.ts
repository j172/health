...
/**
 * POST /api/admin/cool-spots-sync — issue #156. Unlike facilities-import
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    const result = await runCoolSpotsSync();
    return NextResponse.json({ ok: true, ...result });
  } catch (error) {
    return internalErrorResponse(error, "Unknown cool spots sync error");
  }
}

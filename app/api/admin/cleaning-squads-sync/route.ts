...
/**
 * POST /api/admin/cleaning-squads-sync — issue #156. Unlike facilities-import
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    const result = await runCleaningSquadsSync();
    return NextResponse.json({ ok: true, ...result });
  } catch (error) {
    return internalErrorResponse(error, "Unknown cleaning squads sync error");
  }
}

...
import { getCarbonFootprintCoefficientsPage } from "@/lib/server/carbonFootprint/coefficientsQueries";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const { rows, total } = await getCarbonFootprintCoefficientsPage({ keyword, limit, offset });
    return NextResponse.json({ coefficients: rows, total }, { headers: NO_CACHE_HEADERS });
  } catch (error) {
    console.error("GET /api/carbon-footprint-coefficients failed:", error);

...
RSS／AQI／CWA／地震這四項是正式站主機上的 **crontab**（不在此 repo 內管理，只能透過 SSH 到主機用 `crontab -l` 查看/修改），全部經 `127.0.0.1` loopback 呼叫，不會經過對外網域，因此也不受站台前面的防機器人 WAF 影響。`.remote-health-index.php` 另外還有一段訪客觸發的節流邏輯，只是低流量時段的補充，不是主要保證。
